...
map("n", "<leader><leader>Y", "ggvG$\"*y", opts) -- Yank the whole buffer to system clipboard
-- SESSION MANAGEMENT
vim.keymap.set("n", "<leader>qs", function() require("persistence").load() end)   -- load the session for the current directory
vim.keymap.set("n", "<leader>qS", function() require("persistence").select() end) -- select a session to load
vim.keymap.set("n", "<leader>qd", function() require("persistence").stop() end)   -- stop Persistence => session won't be saved on exit
